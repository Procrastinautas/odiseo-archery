...
type TrainingInsert = Database['public']['Tables']['training_sessions']['Insert']
type TrainingUpdate = Database['public']['Tables']['training_sessions']['Update']
type RoundScoreMethod = Database['public']['Tables']['round_scores']['Row']['method']

// ─── Training Sessions ────────────────────────────────────────────────────────

export async function createTrainingSession(data: Omit<TrainingInsert, 'user_id'>) {
...
export async function createRound(trainingSessionId: string) {
  ...
  const { data: round, error } = await supabase
    .from('rounds')
    .insert({ training_session_id: trainingSessionId, round_number: nextNumber })
    .select('id')
    .single()
...
export async function upsertRoundScore(roundId: string, payload: RoundScoreData) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) redirect('/login')
...
export async function createImprovementArea(trainingSessionId: string, comment: string) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) redirect('/login')
