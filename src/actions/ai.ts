...
export async function getTrainingAdvice(trainingSessionId: string): Promise<{ advice: string | null; error?: string }> {
  ...
  const { data: sessions } = await supabase
    ...
    .select('id, type, distance, weather, physical_status, final_thoughts, rounds(round_number, round_scores(method, total_score, tens, xs, misses))')
    .eq('user_id', user.id)
...
export async function getSessionRecap(trainingSessionId: string): Promise<{ recap: string | null; error?: string }> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) redirect('/login')
  ...
  const { data: session } = await supabase
    .from('training_sessions')
    .select('*, rounds(round_number, round_scores(*)), improvement_areas(comment)')
    .eq('id', trainingSessionId)
    .eq('user_id', user.id)
    .single()
