...
export default async function DashboardPage() {
  ...
  const [{ data: profile }, { data: bows }, { data: sessions }, { data: stats }] =
    await Promise.all([
      supabase.from('profiles').select('*').eq('id', user.id).single(),
      supabase.from('bows').select('*').eq('user_id', user.id).limit(1),
      supabase
        .from('scheduled_sessions')
        .select('id, date, time, distance, status, admin_note, locations:location_id(name)')
        .eq('user_id', user.id)
  ...
  return (
    <div className="flex flex-col gap-6 p-4 pt-6">
      ...
      {nextSession ? (
          ...
          <CardContent className="space-y-2">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-semibold">
                  {new Date(nextSession.date + 'T00:00:00').toLocaleDateString('es-CO', {
                    weekday: 'long',
                    day: 'numeric',
                    month: 'long',
                  })}
            ...
            <div className="flex gap-2 pt-1">
              ...
              {nextSession.status === 'confirmed' && (
                <ClientLink href={`/training/new?session=${nextSession.id}`} size="sm" className="flex-1 justify-center">
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center gap-3 py-8 text-center">
            <p className="text-sm text-muted-foreground">No tienes sesiones programadas</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        <ClientLink href="/training/new" variant="outline" className="h-14 flex-col gap-1">
          <Target className="h-5 w-5" />
          <span className="text-xs">Iniciar entreno</span>
        </ClientLink>
        <ClientLink href="/sessions/new" variant="outline" className="h-14 flex-col gap-1">
