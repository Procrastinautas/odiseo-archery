      ...
      <div className="flex flex-col gap-6 p-4">
        ...
        <Card>
          <CardHeader className="pb-3 flex-row items-center justify-between">
            ...
            <Link href="/profile/edit#arcos" className={buttonVariants({ size: 'sm', variant: 'ghost' })}>
              ...
              bows.map((bow, i) => (
                <div key={bow.id}>
                  ...
                  <div className="space-y-1">
                    ...
                    {bow.notes && <p className="text-sm text-muted-foreground">{bow.notes}</p>}
                    {/* Scope marks */}
                    {(bow.scope_marks as unknown as { distance: number; mark_value: string }[])?.length > 0 && (
                      <div className="mt-2">
                        <p className="text-xs font-medium text-muted-foreground mb-1">Miras</p>
                        <div className="flex flex-wrap gap-2">
                          {(bow.scope_marks as unknown as { distance: number; mark_value: string }[]).map((sm) => (
                            <span
                              key={sm.distance}
        ...
        <Card>
          <CardHeader className="pb-3 flex-row items-center justify-between">
            ...
            <Link href="/profile/edit#flechas" className={buttonVariants({ size: 'sm', variant: 'ghost' })}>
              <Plus className="h-4 w-4" />
            </Link>
          </CardHeader>
